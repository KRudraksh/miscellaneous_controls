...
class Drone:
	...
	def __init__(self):
		...
		#PUBLISHERS
		self.publish_pose = rospy.Publisher('/mavros/setpoint_position/local', PoseStamped,queue_size=5)
		self.publish_pos_tar = rospy.Publisher('/mavros/setpoint_raw/local', PositionTarget,queue_size=5)
	...
	'''
	def get_euler_angles(self,imu_data):
		...
		t0 = +2.0 * (w * x + y * z)
		t1 = +1.0 - 2.0 * (x * x + y * y)
		self.roll = np.arctan2(t0, t1)

		t2 = +2.0 * (w * y - z * x)
		t2 = +1.0 if t2 > +1.0 else t2
		t2 = -1.0 if t2 < -1.0 else t2
		self.pitch = np.arcsin(t2)
	...
	def set_acc(self, p_ddot_c):
		sp = PositionTarget()
		sp.coordinate_frame = 1
		sp.type_mask = 3135
		sp.acceleration_or_force.x = p_ddot_c[0]
		sp.acceleration_or_force.y = p_ddot_c[1]
		sp.acceleration_or_force.z = p_ddot_c[2]
...
class smc:
	...
	def controller(self, p, p_dot, p_ddot, p_d, p_dot_d, p_ddot_d, psi_d, p_e, v_e, s_0, t):
		...
		E = self.alpha_1*p_e_u + self.alpha_2*v_e_u + p_ddot_d - self.g + np.dot(self.d_p,p_dot)/self.m
...
def actuate(x, y, z, v_test, v_min, v_max):
	...
	mav.takeoff(5)
	time.sleep(5)
	mav.set_pos(np.array([0,0,10]).transpose())
